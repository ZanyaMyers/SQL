...
SELECT employees.LastName, employees.FirstName, SUM(sales.Quantity) AS totalSold
FROM employees
INNER JOIN sales ON employees.EmployeeID = sales.EmployeeID
GROUP BY EmployeeID;

/* joins: find the name of the department, and the name of the category for Appliances and Games */
SELECT departments.Name AS depName, categories.Name AS catName
FROM departments
INNER JOIN categories on departments.DepartmentID = categories.DepartmentID
WHERE categories.Name = 'Appliances' OR categories.Name = 'Games';
/* joins: find the product name, total # sold, and total price sold,
 for Eagles: Hotel California --You may need to use SUM() */
 SELECT products.Name, SUM(sales.Quantity) as TotalSold, SUM(sales.PricePerUnit * sales.Quantity) AS TotalPriceSold
 FROM products
...
SELECT employees.EmployeeID AS EmployeeID, employees.LastName AS LastName, employees.FirstName AS FirstName, products.Name, SUM(sales.Quantity) AS TotalSales
FROM sales
